...
- (void)makeDownViewUI
{
   // CGFloat photoSize = (ZZ_SCREEN_WIDTH - 60) / 5;
    ...
    _downView = [[UIView alloc]initWithFrame:CGRectMake(0, ZZ_VH - 64, ZZ_VW,  64)];
    if (_themeColor) {
    ...
    UIButton *cancelBtn = [[UIButton alloc]initWithFrame:CGRectMake(30,  20, 50, 30)];
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    [_downView addSubview:cancelBtn];
    
    UIButton *doneBtn = [[UIButton alloc]initWithFrame:CGRectMake(ZZ_VW - 80,  20 , 70, 30)];
    [doneBtn setTitle:@"完成" forState:UIControlStateNormal];
    [doneBtn setTitle:@"请等待" forState:UIControlStateSelected];
    doneBtn.selected = NO;
    [doneBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [doneBtn addTarget:self action:@selector(done) forControlEvents:UIControlEventTouchUpInside];
    _doneBtn = doneBtn;
    [_downView addSubview:_doneBtn];
    
    CGFloat x = (_downView.frame.size.width - 50) / 2;
    UIButton *takePhotoBtn = [[UIButton alloc]initWithFrame:CGRectMake(x, 10 , 50, 50)];
...
- (void)done{
    ...
    _doneBtn.selected = !_doneBtn.selected;
    [_doneBtn setTitle:@"请等待" forState:normal];
    _doneBtn.enabled = NO;
...
- (void)initCameraMain
{
    ...
    NSDictionary *outputSettings = [[NSDictionary alloc] initWithObjectsAndKeys:AVVideoCodecJPEG,AVVideoCodecKey,[NSNumber numberWithInteger:1024],AVVideoWidthKey,[NSNumber numberWithInteger:768],AVVideoHeightKey,AVVideoScalingModeResizeAspectFill,AVVideoScalingModeKey,nil];
    ...
    self.preview.frame = CGRectMake(0, 44, self.view.bounds.size.width, self.view.bounds.size.height-64);
...
- (void)flashOfCamera:(UIButton *)btn
{
    if (btn.tag == 2015) {
        if (![self.device isFlashModeSupported:AVCaptureFlashModeOn]) {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"设备不支持闪光灯" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alert show];
            return;
        }
        if (self.device.flashMode == 0 &&[self.device isFlashModeSupported:AVCaptureFlashModeOn]) {
            [self flashLightModel:^{
                [self.device setFlashMode:AVCaptureFlashModeOn];
            }];
            [btn setImage:Flash_Open_Btn_Pic forState:UIControlStateNormal];
            
        }else if (self.device.flashMode == 1 &&[self.device isFlashModeSupported:AVCaptureFlashModeOff]){
            [self flashLightModel:^{
